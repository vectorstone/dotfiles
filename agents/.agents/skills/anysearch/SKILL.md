...
AnySearch is a unified real-time search service supporting general web search, vertical domain search, parallel batch search, and full-page content extraction. It exposes a single JSON-RPC 2.0 endpoint and requires no MCP server installation. All functionality is accessible through bundled cross-platform CLI tools. For macOS/Linux dotfile installs, prefer the portable launcher at `<skill_dir>/scripts/anysearch`; it resolves the skill directory and chooses the best available runtime without hard-coded machine paths. Use the configured runtime directly for routine `search`, `batch_search`, `extract`, and `list_domains` calls; run the `doc` command only when the CLI interface is unknown or recovery information is needed (see Recommended Entry Point).
...
Prefer direct CLI invocation through `<skill_dir>/scripts/anysearch` on macOS and Linux. If `<skill_dir>/runtime.conf` exists and the requested command shape is already obvious (`search`, `batch_search`, `extract`, or `list_domains`), the agent SHOULD use the configured command directly and SHOULD NOT run `doc` on every activation. Run `doc` only when the CLI interface is unknown, a command fails due to argument/schema uncertainty, the skill was just installed/updated, or vertical-domain constraints require the complete reference. The `doc` command is offline and remains available for recovery, but repeated metadata reads waste tool calls and tokens.

The portable launcher selects runtimes in this order:

1. `python3` with the `requests` package
2. `python` with the `requests` package
3. `node`
4. `bash` with `jq` and `curl`

### Command Cheat Sheet

Use these exact command shapes for routine calls. Replace `<cmd>` with the command from `runtime.conf`; for portable macOS/Linux installs, use `<skill_dir>/scripts/anysearch`. Do not invent extra output-format flags.
...
| Runtime | Command |
|---------|---------|
| Auto launcher | `<skill_dir>/scripts/anysearch doc` |
...
If `<skill_dir>/runtime.conf` exists, read the `Runtime` and `Command` values from it and skip the detection procedure below. Treat this as the normal fast path for routine searches. In shared dotfiles, the recommended command is an absolute path to `<skill_dir>/scripts/anysearch` in the local checkout; do not commit `runtime.conf`. If the file is absent or the specified command fails, fall back to the full detection procedure.

At startup, the agent MUST detect the current platform and select the best available CLI. On macOS and Linux, first try the bundled launcher:

```
<skill_dir>/scripts/anysearch doc
```

If the launcher is unavailable, select the best available CLI manually. The manual priority order is:
...
| Runtime | Invocation |
|---------|-----------|
| Auto launcher | `<skill_dir>/scripts/anysearch <command> [options]` |
