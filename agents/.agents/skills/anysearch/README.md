...
After installation, run the portable launcher entry test first. It works on macOS and Linux, including Arch Linux, because it resolves the skill directory at runtime and falls back from Python to Node.js to Bash as needed:
...
The `doc` command should print `# AnySearch Interface Specification (for AI Agent)`.

If the launcher is unavailable, probe the platform and run the runtime-specific entry test:
...
Launcher priority: **Python with requests > Node.js > Bash with jq and curl**

Manual runtime priority: **Python > Node.js > Shell**
...
Run the `doc` command with the launcher first, then with **each available** runtime if you need to diagnose a runtime-specific problem:

```bash
# Portable launcher for macOS and Linux
<skill_dir>/scripts/anysearch doc
```
...
Based on the entry test results, write the recommended runtime to `<skill_dir>/runtime.conf`. For shared dotfiles on macOS and Linux, prefer the portable launcher:

```bash
echo "Runtime: Auto" > <skill_dir>/runtime.conf
echo "Command: <skill_dir>/scripts/anysearch" >> <skill_dir>/runtime.conf
```

If your agent cannot expand `<skill_dir>`, write the absolute path for the local machine. Keep `runtime.conf` ignored and do not commit machine-specific paths.
...
After `runtime.conf` exists, agents should use the stored `Command` directly for routine calls instead of running `doc` before every search. For portable macOS/Linux installs, use:
...
<skill_dir>/scripts/anysearch extract --url "https://example.com/page"
```

For example, if `runtime.conf` contains `Command: python3 <skill_dir>/scripts/anysearch_cli.py`, use:
...
└── scripts/
    ...
    └── anysearch              # macOS/Linux portable runtime launcher
```
